...
func Login(user, pass, id string) bool {
	...
	for _, line := range lines {
		...
		if len(parts) == 5 && parts[1] == "U" { // Verifica que es un usuario
			userID, userType, username, group, password := parts[0], parts[1], parts[2], parts[3], parts[4]
...
// FindFileInode busca el inodo de un archivo por su ruta dentro del sistema de archivos
func FindFileInode(file *os.File, superblock Ext2.Superblock, filePath string) int {
	// Leer el inodo raíz
	var rootInode Ext2.Inode
	inodeStart := int64(superblock.S_inode_start)
	if err := Utilities.ReadObject(file, &rootInode, inodeStart); err != nil {
		fmt.Println("Error al leer el inodo raíz.")
		return -1
	}

	// Buscar el archivo dentro de los bloques de datos del inodo raíz
	for _, blockIndex := range rootInode.I_block {
		...
		// Buscar el nombre del archivo dentro del bloque
